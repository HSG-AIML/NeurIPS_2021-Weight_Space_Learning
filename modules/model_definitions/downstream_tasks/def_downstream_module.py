...
sys.path.append("./../")
from model_definitions.def_FastTensorDataLoader import FastTensorDataLoader
import numpy as np
...
class DownstreamTaskLearner:
    ...
    def eval_dstasks(self, model, trainset, testset, valset=None, batch_size=100):
        ...
        if valset is not None:
            ...
            z_val = self.map_embeddings(
                weights=w_val, model=model, batch_size=batch_size,
            )
        else:
            z_val = None
    ...
    def map_embeddings(
        self, weights: torch.Tensor, model: nn.Module, batch_size: int = 100,
    ) -> torch.Tensor:
        """
        computes the
        """
